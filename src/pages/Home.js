...
const Home = ({ setActive, user, active }) => {
  ...
  const updateState = (docSnapshot) => {
    ...
    if (!isCollectionEmpty) {
      ...
      toast.info("No more News to display");
      setHide(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure wanted to delete that News ?")) {
      try {
        setLoading(true);
      ...
      <div className="container padding">
        <div className="row mx-0">
          ...
          <div className="col-md-8">
            <div className="blog-heading text-start py-2 mb-4">Daily News</div>
            {blogs.length === 0 && location.pathname !== "/" && (
              <>
                <h4>
                  No News found with search keyword:{" "}
